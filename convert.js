...
function processSegments(pathLength, xml) {
...
function pointSegmentString(point, segment) {
...
function addMulti(map, key, value) {
    ...
    arr === null || arr === void 0 ? void 0 : arr.push(value);
}
function removeMulti(map, key, value, equality) {
    if (!map.has(key)) {
        return;
    }
    const arr = map.get(key);
...
function segmentOtherPoint(segment, point) {
...
function smallestPoint(pointToValue, points) {
    let sx = undefined;
    let sy = undefined;
...
function convertToPoly(segments) {
    let pointToValue = new Map();
    let pointToSegments = new Map();
    let pointSegmentAngle = new Map();
    let boundarySegments = new Set();
...
function convertToCode(poly) {
    ...
    s += `\n`;
    const regular = document.getElementById("doCheckRegular");
    if (regular.checked) {
    ...
    const unimodular = document.getElementById("doCheckUnimodular");
    ...
    const visualize = document.getElementById("doVisualize");
